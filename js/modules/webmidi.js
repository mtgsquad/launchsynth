...
(function (exports) {
  ...
  class e {
    ...
    addListener(n, r, i = {}) {
      if (
        ...
        (n instanceof String && n.length < 1) ||
        ("string" != typeof n && !(n instanceof String) && n !== e.ANY_EVENT)
      )
        throw new TypeError(
      ...
      return (
        this.eventMap[n] || (this.eventMap[n] = []),
        i.prepend ? this.eventMap[n].unshift(s) : this.eventMap[n].push(s),
        s
      );
    }
    ...
    hasListener(n, r) {
      if (void 0 === n)
        return (
          !!(
            this.eventMap[e.ANY_EVENT] && this.eventMap[e.ANY_EVENT].length > 0
    ...
    emit(t, ...n) {
      ...
      return (
        this.eventMap[t] && (i = i.concat(this.eventMap[t])),
        i.forEach((e) => {
          if (e.suspended) return;
          let t = [...n];
          Array.isArray(e.arguments) && (t = t.concat(e.arguments)),
            e.remaining > 0 &&
              (r.push(e.callback.apply(e.context, t)), e.count++),
      ...
      return (
        ...
        new Promise((n, r) => {
          ...
          t.duration !== 1 / 0 &&
            (i = setTimeout(() => {
              s.remove(),
                r("The duration expired before the event was emitted.");
            }, t.duration));
        })
      );
    }
  ...
  class t {
    constructor(t, n, r, i = {}) {
      if ("string" != typeof t && !(t instanceof String) && t !== e.ANY_EVENT)
        throw new TypeError(
          "The 'event' parameter must be a string or EventEmitter.ANY_EVENT."
        );
      ...
      void 0 === i.arguments ||
        ...
        (i = Object.assign(
          ...
          1 / 0 && setTimeout(() => this.remove(), i.duration),
        ...
        (this.event = t),
        (this.remaining =
          parseInt(i.remaining) >= 1 ? parseInt(i.remaining) : 1 / 0),
        (this.suspended = !1),
        (this.target = n);
    }
  ...
  class Utilities {
    ...
    static getCcNameByNumber(number) {
      return Utilities.getPropertyByValue(
  ...
  class OutputChannel extends e {
    ...
    sendKeyAftertouch(target, pressure, options = {}) {
      ...
      const offset =
        wm.octaveOffset + this.output.octaveOffset + this.octaveOffset; // Make sure we are dealing with an array

      if (!Array.isArray(target)) target = [target];
      Utilities.buildNoteArray(target).forEach((n) => {
        this.send(
          [
            (Enumerations.MIDI_CHANNEL_MESSAGES.keyaftertouch << 4) +
              (this.number - 1),
    ...
    sendControlChange(controller, value, options = {}) {
      ...
      value.forEach((item, index) => {
        this.send(
          [
            (Enumerations.MIDI_CHANNEL_MESSAGES.controlchange << 4) +
              (this.number - 1),
            controller + index * 32,
            value[index],
          ],
    ...
    sendRpnDecrement(parameter, options = {}) {
      if (!Array.isArray(parameter))
        parameter = Enumerations.MIDI_REGISTERED_PARAMETERS[parameter];
    ...
    playNote(note, options = {}) {
      ...
      if (
        options.duration > 0 &&
        isFinite(String(options.duration).trim() || NaN)
      ) {
        let noteOffOptions = {
          time:
            (Utilities.toTimestamp(options.time) || wm.time) + options.duration,
    ...
    sendNoteOff(note, options = {}) {
      ...
      Utilities.buildNoteArray(note, {
        ...
        this.send(
          [
            (Enumerations.MIDI_CHANNEL_MESSAGES.noteoff << 4) +
              (this.number - 1),
            n.getOffsetNumber(offset),
            n.rawRelease,
          ],
    ...
    sendNoteOn(note, options = {}) {
      ...
      Utilities.buildNoteArray(note, {
        ...
        this.send(
          [
            (Enumerations.MIDI_CHANNEL_MESSAGES.noteon << 4) +
              (this.number - 1),
            n.getOffsetNumber(offset),
            n.rawAttack,
          ],
    ...
    sendChannelMode(command, value = 0, options = {}) {
      ...
      if (typeof command === "string")
        command = Enumerations.MIDI_CHANNEL_MODE_MESSAGES[command];
    ...
    sendChannelAftertouch(pressure, options = {}) {
      ...
      this.send(
        [
          (Enumerations.MIDI_CHANNEL_MESSAGES.channelaftertouch << 4) +
            (this.number - 1),
    ...
    sendPitchBend(value, options = {}) {
      ...
      this.send(
        [
          (Enumerations.MIDI_CHANNEL_MESSAGES.pitchbend << 4) +
            (this.number - 1),
          lsb,
          msb,
        ],
    ...
    sendProgramChange(program, options = {}) {
      ...
      this.send(
        [
          (Enumerations.MIDI_CHANNEL_MESSAGES.programchange << 4) +
            (this.number - 1),
          program,
        ],
        {
    ...
    sendRpnValue(rpn, data, options = {}) {
      if (!Array.isArray(rpn))
        rpn = Enumerations.MIDI_REGISTERED_PARAMETERS[rpn];
  ...
  class Output extends e {
    ...
    sendSysex(manufacturer, data = [], options = {}) {
      ...
      if (data instanceof Uint8Array) {
        const merged = new Uint8Array(
          1 + manufacturer.length + data.length + 1
        );
        ...
        const merged = manufacturer.concat(
          data,
    ...
    sendNoteOn(note, options = {}, legacy = {}) {
      ...
      if (options.channels == undefined)
        options.channels = Enumerations.MIDI_CHANNEL_NUMBERS; // This actually supports passing a Note object even if, semantically, this does not make sense.
  ...
  class InputChannel extends e {
    ...
    _parseEventForStandardMessages(e) {
      ...
      if (
        ...
        if (event.message.dataBytes[0] >= 120)
          this._parseChannelModeMessage(event); // Parse the inbound event to see if its part of an RPN/NRPN sequence
    ...
    _isRpnOrNrpnController(controller) {
      return (
        controller ===
          Enumerations.MIDI_CONTROL_CHANGE_MESSAGES.dataentrycoarse || //   6
        controller ===
          Enumerations.MIDI_CONTROL_CHANGE_MESSAGES.dataentryfine || //  38
        controller ===
          Enumerations.MIDI_CONTROL_CHANGE_MESSAGES.databuttonincrement || //  96
        controller ===
          Enumerations.MIDI_CONTROL_CHANGE_MESSAGES.databuttondecrement || //  97
        controller ===
          Enumerations.MIDI_CONTROL_CHANGE_MESSAGES
            .nonregisteredparametercoarse || //  98
        controller ===
          Enumerations.MIDI_CONTROL_CHANGE_MESSAGES
            .nonregisteredparameterfine || //  99
        controller ===
          Enumerations.MIDI_CONTROL_CHANGE_MESSAGES.registeredparametercoarse || // 100
        controller ===
          Enumerations.MIDI_CONTROL_CHANGE_MESSAGES.registeredparameterfine
      ); // 101
    }
    ...
    _dispatchParameterNumberEvent(type, paramMsb, paramLsb, e) {
      ...
      if (type === "rpn") {
        event.parameter = Object.keys(
          Enumerations.MIDI_REGISTERED_PARAMETERS
        ).find((key) => {
          return (
            Enumerations.MIDI_REGISTERED_PARAMETERS[key][0] === paramMsb &&
    ...
    static get EVENTS() {
      return [
        ...
        "programchange",
        "channelaftertouch",
        "pitchbend", // MIDI channel mode events
        "allnotesoff",
        "allsoundoff",
        "localcontrol",
        "monomode",
        "omnimode",
        "resetallcontrollers", // RPN/NRPN events
        "nrpn",
        "nrpn-dataentrycoarse",
        "nrpn-dataentryfine",
        "nrpn-databuttonincrement",
  ...
  class Input extends e {
    ...
    removeListener(event, listener, options = {}) {
      ...
      if (options.channels === undefined)
        options.channels = Enumerations.MIDI_CHANNEL_NUMBERS; // If the event is not specified, remove everything (channel-specific and input-wide)!
